...
use crate::formation_rpc::{MessageHeader, DirectedMessageRequest, DirectedMessageResponse};
use tonic::{Response, Status};
use base64::{engine::general_purpose::URL_SAFE_NO_PAD, Engine as _};
use alloy::primitives::{Address, hex::FromHex};
use sha3::{Digest, Sha3_256};

pub async fn publish_direct_message<N>(
    node: &N,
    request: &DirectedMessageRequest
) -> Result<Response<DirectedMessageResponse>, Status> 
where
    N: Node + Send + Sync
{
    ...
    node.publish(
        Box::new(QuorumTopic),
        Box::new(Event::QuorumEvent(QuorumEvent::DirectMessage {
    ...
    let network_gossip_response = DirectedMessageResponse {
        header: Some(header),
        original_message_id,
        ack: true,
