...
use crate::formation_rpc::{MessageHeader, NetworkGossipRequest, NetworkGossipResponse};
use tonic::{Response, Status};
use base64::{engine::general_purpose::URL_SAFE_NO_PAD, Engine as _};
use sha3::{Digest, Sha3_256};
use alloy::primitives::{Address, hex::FromHex};

pub async fn publish_network_gossip<N>(
    node: &N,
    request: &NetworkGossipRequest
) -> Result<Response<NetworkGossipResponse>, Status> 
where
    N: Node + Send + Sync
{
    ...
    node.publish(
        Box::new(QuorumTopic),
        Box::new(Event::QuorumEvent(QuorumEvent::NetworkGossip {
    ...
    let network_gossip_response = NetworkGossipResponse {
        header: Some(header),
        original_message_id,
        ack: true,
